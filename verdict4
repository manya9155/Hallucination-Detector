    ...
    for claim in claims:
        ...
        if attr in ["director", "actor"]:
            if tmdb and "credits" in tmdb:
                people = [c["name"] for c in tmdb["credits"]["crew"]] if attr=="director" else [c["name"] for c in tmdb["credits"]["cast"][:50]]
                if fuzzy_match(val, people): verdicts.append(True)
                sources_used.add("TMDB")
            if omdb:
                key = "Director" if attr=="director" else "Actors"
                if omdb.get(key) and fuzzy_match(val, omdb.get(key).split(",")): verdicts.append(True)
                sources_used.add("OMDB")
            if wikidata:
                key = "directorLabel" if attr=="director" else "castLabel"
                if any(fuzzy_match(val, [entry[key]["value"]]) for entry in wikidata if key in entry): verdicts.append(True)
...
if __name__=="__main__":
    sentence = "Avengers is an action movie"
